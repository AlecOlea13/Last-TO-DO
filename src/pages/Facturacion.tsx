...
const CLAVES_PROD = [
  { clave: "80101500", desc: "Servicios de mantenimiento y reparación" },
  { clave: "80141600", desc: "Servicios de transporte de carga" },
  { clave: "78101801", desc: "Servicios de arrendamiento de equipo" },
  { clave: "84111506", desc: "Servicios de facturación" },
  { clave: "25174700", desc: "Montacargas" },
    ...
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && results.length === 0 && !loading && query.length >= 2 && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 9999, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "10px 14px", color: "var(--text-muted)", fontSize: "0.85rem" }}>
          Sin resultados — puedes capturar los datos manualmente
        </div>
      )}
    </div>
  );
...
export default function Facturacion() {
  ...
  function onClienteSelect(c: any) {
    setReceptor(prev => ({
      ...
      cp:            c.cp            ?? c.codigoPostal ?? prev.cp,
          ...
          <div className="modal" style={{ maxWidth: 800, width: "96vw" }}>
                  ...
                  <div key={i} style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: 12 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                      <div className="form-group" style={{ margin: 0 }}>
                        ...
                        <select className="form-select" value={p.claveProdServ} onChange={e => updatePartida(i, "claveProdServ", e.target.value)}>
                          {CLAVES_PROD.map(c => <option key={c.clave} value={c.clave}>{c.clave} — {c.desc}</option>)}
                        </select>
                      </div>
                      <div className="form-group" style={{ margin: 0 }}>
            ...
            {metodoPago === "PPD" && (
              <div style={{ padding: "10px 14px", background: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: "var(--radius-sm)", fontSize: "0.82rem", color: "var(--blue)", marginTop: 8 }}>
                💡 Con método <strong>PPD</strong> deberás emitir un <strong>Recibo Electrónico de Pago (REP)</strong> cada vez que el cliente realice un pago parcial o total.
              </div>
            )}
        ...
        <div className="modal-overlay" onMouseDown={e => { if (e.target === e.currentTarget) setModalCancelar(null); }}>
          <div className="modal" style={{ maxWidth: 420 }}>
            ...
            <div style={{ padding: "10px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid var(--red)", borderRadius: "var(--radius-sm)", fontSize: "0.82rem", color: "var(--red)", marginBottom: 12 }}>
              ⚠️ Esta acción cancela el CFDI ante el SAT. El receptor recibirá una notificación. Si el motivo es <strong>01</strong> necesitas emitir un CFDI de sustitución.
            </div>
            <div className="modal-footer">
